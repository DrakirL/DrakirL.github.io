...
**Spiritual**
+ Infernal? Unholy? Corrupting?
+ Necrotic
+ Radiant? Holy? Smithing?
#### <br>
...
A creature can have an amount of Exhaustion up to their Maximum Health and remain active. If a creature's Exhaustion EVER exceeds its Maximum Health, it immidiately dies (or falls unconcious for an indeterminate time, if the GM is feeling merciful).<br>
...
+ Deal 1* damage to the target. The (normally Physical) type of the damage dealt depends is decided based on vibes.
+ Grapple the target.
...
*Extended Range: You can attack any target within the given range (your Throwing Range unless otherwise specified) as normal. You can attack a target outside that range by increasing the Difficulty by 1 for each pace the distance exceeds the range by.*

#### Brace Action
Prepare for the worst and steel your nerves by using any number of Actions. <br>
Until you've regained the Action, you add a +1 bonus to your Defence, your Awareness, your Martial Might, your Magic Might, and any Save you make. <br>
...
#### Boost Reaction
When another creature within your reach makes a Test of Skill that takes time measured in Actions, you can support them by using any number of your Actions.<br>
Roll a d8 for each used Action and add their results to the total.<br>
*For roleplay reasons, you have to descibe how your efforts are helping to do so. Have good reasons, and your declaration could be allowed some room for vagueness or generous interpretation*
<br><br>
Alternatively, when a allied creature makes an Attack against a target that is within your Reach, you can add d8s to the Attacker's total in the same way.

#### Block Reaction
When you are targeted by an Attack, you can intentionally push back against it by using any number of Actions.<br>
Roll a d8 for each used Action and subract the sum of their results from the incoming Attack's total.<br>
*Non-players normally don't use this reaction. Hardy monsters don't need to, instead saving their actions for bigger things, and opponents without advanced combat training don't know how to.*<br>
*If an opponent IS employing advanced combat training, this might be used extensively >:)*
...
When a creature makes a Test targeting something that is Obscured from them, each Action Die is rolled with Disadvantage.
...
If a creature is breathing, it can start holding its breath as part of any other Action by making a Difficulty 0 Test of Endurance, bypassing its need to breathe for up to 1* Rounds.
...
If a creature is forcefully pushed into a cushioning surface, the damage it halved. If a creature is pushed into a different creature no smaller than itself, it counts as a cushioning surface.<br>
...
If you lack the Proficiency associated with a given weapon, using a weapon properly becomes more difficult. You can still follow the restrictions and gain the benefits, but you can't add your Combat Mastery to the result.
...
+ **Ranged (X):** To use a Ranged weapon properly, you must use it to make a Martial Range Attack using the given range. *(If scaled up, multiply X by the intended wielder's reach)*
...
### Small
+ Buckler: Damage (1* Bludgeoning). Bash. Block (+1? d8?) Can be used once without using an Action?
+ Targe: Uses Strength. Damage (2* Bludgeoning). Bash. Block (+2? d10?) Hefty (6)?

### Large
+ Heater: Uses Strength. Damage (2* Bludgeoning). Bash. Block (+2? d12?) Hefty (8)? Bulky. Can be used to create Partial Cover.
+ Tower: Uses Strength. Damage (3* Bludgeoning). Bash. Block (+3? d12?) Hefty (10)? Bulky. Can be used to create Full Cover.

### Shield Properties
+ **Block (X):** When you use a Block shield to Block, add the specified bonus to your roll?.
+ **Bulky:** A Bulky shield always adds to the wielder's Defence. When used to Block an Attack aimed at creature other than the wielder, the Attack is redirected to the wielder instead.
+ **Hefty:** DOWNSIDE? RESTRICTION?
